...
よってC2インスタンスが生成された際に変数@fooには文字列"foo"が代入されていまので"foo"と表示されます。
'
...
m_1メソッドではC1インスタンスに対してメソッドであるm_2メソッドを呼び出しています。
...
$o_commentary6 =
  'この問題ではC1クラスを継承したC1クラスのインスタンスをレシーバとしてm_2メソッドを呼び出しています。
...
$o_commentary11 = 
  'この問題ではC1クラスを継承したC3クラスに対してinstance_methodsを呼んでいます。
instance_methodsメソッドはあるクラスのインスタンスメソッドの一覧を配列で得ることができるメソッドです。
またfalseをつけることでそのモジュールで定義されたインスタンスメソッドのみを返します。

instance_methodsの返り値はArray（配列）となりますのでその配列の中にm_1メソッドがあるかをinclude?メソッドで判断しています。
...
そしてここで使用されているaliasは別名をつけるための構文です。
よってここではC1クラスで定義されているm_1メソッドをC1クラスを継承したC2クラスで"m_2"という別名をつけているので[:m_2]と表示されます。

元々定義されているm_1メソッドが使用出来いわけではないこと、alias式には文字列は指定できない事、カンマは不要であるという認識が必要です。'
...
さらに補足するとModule.instance_methods.include?(:new)とするとfalseが返ってきます。
Moduleクラスのインスタンスメソッドにnewが定義されていない為、例外が発生します。'
...
ここではモジュールM1とM2をクラスC1でインクルードしています。
includeメソッドによって取り込まれたモジュールの優先順位はそのクラスよりも後になります。
...
ここではモジュールM1とM2をクラスC1でインクルードしています。
prependメソッドによって取り込まれたモジュールの優先順位はそのクラスよりも先になります。

prependメソッドで呼ばれたモジュールが優先される為、ここでは"M2 m_1"と表示されます。
...
$o_commentary24 = 
  'ここではクラスC1のインスタンスであるc1に対してm_2という特異メソッドを定義しています。
c1に対してのみm_2メソッドを定義していることに注意が必要です。
...
そしてこの問題ではC1とC2とC3クラスに対してnewメソッドを呼んで3回initializeメソッドが呼ばれています。
サブクラス(ここではC2)でinitializeメソッドを定義しなかった場合、initializeメソッドも継承されます。
...
よって、@@count += 1 が2回実行されるので「2」と表示されます。'
...
その為、@@count += 1 が実行されるのは1回なので「1」と表示されます。'
...
その為、@@count += 1 が2回実行されるので「2」と表示されます。'

$o_commentary30 =
  'prependメソッドはモジュールのメソッドをクラスに取り込む為のメソッドになります。
ここではモジュールM1をクラスC2に取り込んでいます。
prependメソッドを使用した際のM1の優先順位は呼び出したクラスよりも先になります。
C2クラスのインスタンスであるc2をレシーバとしてm_1メソッドを呼び出しています。

継承チェーンを確認するとM1モジュールのm_1が呼び出されますがその中でさらにsuperが呼ばれています。
継承チェーン上でM1よりも優先されるクラスもしくはモジュールはC2となります。
よって
"C2"
"M1"
と表示されます。
...
$o_commentary31 =
  'includeメソッドはモジュールのメソッドをクラスに取り込む為のメソッドになります。
ここではモジュールM1をクラスC2に取り込んでいます。
C2クラスのインスタンスc2をレシーバとしてm_1メソッドをよんでいますが、継承チェーンを確認すると１番優先順位が高いのはC2なのでC2のm_1メソッドが実行されまます。
しかしその中でさらにsuperが呼ばれているのでM1モジュールのm_1メソッドが呼ばれます。
